...
function MoveSlot({
  ...
  const visibleCount = Math.min(filtered.length, 60);
  const onKey = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setHi((h) => Math.min(h + 1, visibleCount - 1)); }
  ...
  return (
    <div className="relative" ref={ref}>
      <div
        className={cn(
          "flex items-center gap-2.75 w-full bg-input-bg border rounded-[9px] px-3 py-2.25 transition-all duration-140",
          open ? "border-accent shadow-[0_0_0_3px_var(--accent-soft)] bg-surface" : "border-line hover:border-muted",
        )}
      >
        <button
          type="button"
          className="flex items-center gap-2.75 flex-1 min-w-0 text-left bg-transparent border-0 p-0"
          onClick={toggle}
        >
