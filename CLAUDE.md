...
- **Server** (`server/`) — fully implemented. Express + Drizzle + PostgreSQL. Rate limiting, `helmet`, and JWT revocation (`users.token_version`) harden auth. No test runner. See `server/CLAUDE.md` for API surface, security model, validation strategy, and schema details.
...
- **Pokédex Community layer** — real, database-backed wiki on top of the static Pokédex (competitive movesets, contributors, edit history, propose/vote/moderate). Spans both sides: `server/src/services/analysisService.ts` + the `pokemon_analyses`/`analysis_sets`/`analysis_revisions`/`analysis_proposals`/`proposal_votes` tables, surfaced through `client/hooks/usePokemonAnalysis.ts` on `/pokedex/[slug]`. Only `tyranitar` has any content today — everyone else's Community section is genuinely empty, not a bug. See server/CLAUDE.md's "Pokémon Analyses (Community Layer)" section for the concurrency-safety details (atomic proposal accept/reject, etc.) before touching that code.
